...
int main (int argc, char *argv[])
{
	...
	if(taskid == 0) {
		...
		local_p.resize(primes.size() / numtasks);
		
		MPI_Scatter(primes.data()+(primes.size() % numtasks), primes.size() / numtasks, MPI_UINT64_T,\
		local_p.data(), primes.size() / numtasks, MPI_UINT64_T, 0, MPI_COMM_WORLD);
